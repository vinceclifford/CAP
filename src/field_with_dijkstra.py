...
def construct_entire_graph(robot, target: StaticCircle, obstacles, width, height, alpha=1, temp=1):
    ...
    Returns: (node, node): Returns the starting and goal node of the graph. We only need these two. All other nodes
        ...
        for y in range(0, height):
            ...
            for point in neighboring_points:
                if point not in map_koordinates_to_field_value:
                    ...
                    node_looking_at.add_neighbours(map_koordinates_to_node[point], map_koordinates_to_field_value[point])
