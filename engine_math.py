...
def calculate_total_force(robot, target, obstacle_set): 
    ...
    print(sum_repulsion_force)
    attraction_force = calculate_attraction_force(robot, target)
    return (sum_repulsion_force[0] + attraction_force[0], sum_repulsion_force[1] + attraction_force[1])

def calculate_attraction(robot, target): 
    if norm_subtraction(robot.vektor, target.vektor) <= SIGMA_ZERO: 
        return 0.5 * XI * (norm_subtraction(robot.vektor, target.vektor)**2)
    return XI * norm_subtraction(robot.vektor, target.vektor) * SIGMA_ZERO - 0.5 * XI * SIGMA_ZERO * SIGMA_ZERO
...
def calculate_attraction_force(robot, target): 
    x_result = -XI * (robot.vektor[0] - target.vektor[0])
    y_result = -XI * (robot.vektor[1] - target.vektor[1])
...
def calculate_single_repulsion(robot, obstacle): 
    if norm_subtraction(robot.vektor, obstacle.vektor) > obstacle.radius_of_influence: 
        return 0.0
    return 0.5 * ETA * ((1.0/norm_subtraction(robot.vektor, obstacle.vektor)) - 1.0/obstacle.radius_of_influence) ** 2 
...
def calculate_single_repulsion_force(robot, obstacle): 
    ...
    coefficients = ETA * (1/distance - 1/obstacle.radius_of_influence) * 1/(distance ** 2)
