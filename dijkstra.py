...
def dijkstra_on_graph_with_dictionary(tensor, start, goal, width, height): 
    start_node, goal_node = construct_graph_from_tensor(tensor, start, goal, width, height)
    return dijkstra(start_node, goal_node)


def dijkstra_on_tensor_with_tensor(tensor, start, goal, width, height): 
    """Summary of dijkstra_on_tensor_with_tensor(): Instead of performing Dijkstras Algorithm on an actual graph, we will emulate a graph. In practice this means not actually creating any nodes or edges. The weight of the edges to each neighbour are given by the tensor which we computed previously. The neighbour of a current node are simple the nodes to its left, right, up and down direction. Special edge cases may apply to the edges of the environment. 
    ...
    # The sequence of start[1], start[0] is correct. It should not be start[0], start[1] due to the fact that we first specify the row in tensor lookups
    ...
    while priority_queue: 
        ...
        for direction in directions: 
            ...
            if next_x >= 0 and next_x < width and next_y >= 0 and next_y < height: 
                ...
                if lookup_tensor[next_y, next_x, 0] == torch.finfo(torch.float32).max or distance < lookup_tensor[next_y, next_x, 0].item(): 
...
def dijkstra_on_tensor_with_dictionary(tensor, start, goal, width, height): 
    """Summary of dijkstra_on_tensor_with_tensor(): Instead of performing Dijkstras Algorithm on an actual graph, we will emulate a graph. In practice this means not actually creating any nodes or edges. The weight of the edges to each neighbour are given by the tensor which we computed previously. The neighbour of a current node are simple the nodes to its left, right, up and down direction. Special edge cases may apply to the edges of the environment. 
...
def dijkstra_on_tensor_with_array(tensor, start, goal, width, height): 
    lookup_array = np.full((3,height, width), -1)
    ...
    while priority_queue: 
        ...
        if coordinate_tuple == goal:
            ...
            while coordinate_tuple != (-1, -1): 
                shortest_path.append(coordinate_tuple)
                coordinate_tuple = (lookup_array[1, coordinate_tuple[1], coordinate_tuple[0]], lookup_array[2, coordinate_tuple[1] , coordinate_tuple[0]])
...
def dijkstra_on_nparray_with_dictionary(tensor, start, goal, width, height): 
    """Summary of dijkstra_on_tensor_with_tensor(): Instead of performing Dijkstras Algorithm on an actual graph, we will emulate a graph. In practice this means not actually creating any nodes or edges. The weight of the edges to each neighbour are given by the tensor which we computed previously. The neighbour of a current node are simple the nodes to its left, right, up and down direction. Special edge cases may apply to the edges of the environment. 
...
def dijkstra_on_nparray_with_dictionary_without_detach(tensor, start, goal, width, height): 
    """Summary of dijkstra_on_tensor_with_tensor(): Instead of performing Dijkstras Algorithm on an actual graph, we will emulate a graph. In practice this means not actually creating any nodes or edges. The weight of the edges to each neighbour are given by the tensor which we computed previously. The neighbour of a current node are simple the nodes to its left, right, up and down direction. Special edge cases may apply to the edges of the environment. 
...
def dijkstra_on_nparray_with_array(tensor, start, goal, width, height): 
    np_array = tensor.detach().numpy()
    lookup_array = np.full((3,height, width), -1)
    lookup_array[0, start[1], start[0]] = 0 
    directions = [(-1, 0), (1, 0), (0, 1), (0, -1)]
...
def dijkstra_on_nparray_with_two_array(tensor, start, goal, width, height): 
    np_array = tensor.detach().numpy()
    cost_array = np.full((height, width), -1)
    parent_array = np.full((2, height, width), -1, dtype=np.int32)
    ...
    while priority_queue: 
        ...
        if coordinate_tuple == goal:
            ...
            while coordinate_tuple != (-1, -1): 
                shortest_path.append(coordinate_tuple)
                coordinate_tuple = (parent_array[0, coordinate_tuple[1], coordinate_tuple[0]], parent_array[1, coordinate_tuple[1] , coordinate_tuple[0]])
